...
### 3. Application Config Version (`config/app.yml`)

Base version only (used by Rails app):
```yaml
shared: &shared
  version: '4.8.0'
```
...
### 5. Helm Chart Version (`charts/chatwoot/Chart.yaml`)
...
### 6. Helm Values (`charts/chatwoot/values.yaml`)
...
# Update config/app.yml version (line 2) - base version only
# Edit manually to set version: '4.8.0'
...
grep -r "4.8.0" VERSION_CW VERSION_CWCTL config/app.yml package.json charts/chatwoot/Chart.yaml charts/chatwoot/values.yaml
...
- [ ] Version updated in `config/app.yml`
