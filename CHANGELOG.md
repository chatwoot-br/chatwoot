...
## [v4.10.1+4] - 2026-03-15

### Added

- **Auto-onboard admin**: Auto-create SuperAdmin user from `CHATWOOT_ADMIN_EMAIL` / `CHATWOOT_ADMIN_PASSWORD` env vars during `db:prepare`, skipping the browser-based onboarding screen
- **E2E testing**: Docker Compose stack (`docker-compose.e2e.yaml`) for local production-mode validation

### Fixed

- **Dockerfile**: Tolerate missing `.git` directory in worktrees (fallback for `git_sha`)

## [v4.10.1+3] - 2026-01-24

### Added

- **WhatsApp JID Validation**: Add `processable_chat_jid?` helper for filtering valid chat JIDs

### Fixed

- **WhatsApp History Sync**: Validate JIDs and add resilient error handling in history sync
- **DevContainer**: Improve Dockerfile permissions and enhance setup script for port visibility

## [v4.10.1+2] - 2026-01-21

### Fixed

- **WhatsApp Duplicate Conversations**: Fix race condition that created duplicate conversations when concurrent webhook jobs processed messages for the same contact simultaneously. Added pessimistic locking on `contact_inbox` record in `set_conversation` method.

## [v4.10.1+1] - 2026-01-20
